...
// A line that must contain a fake, key-shaped value (test fixture) can end with: guard-allow-secret
...
// No dependencies, so it works before any install step.
...
// The file-name and content rules live in scripts/key-rules.mjs, shared with other scanners, and the content
// patterns themselves live in scripts/key-patterns.mjs.
const problems = [];
const skippedBinary = [];
const notScanned = [];
let scanned = 0;
...
if (problems.length) {
  console.error(`\n✖ ${all ? 'guard-keys --all failed' : 'Commit blocked'}: possible secrets or unscannable files found (this repository must never contain real keys).\n`);
  for (const p of problems) console.error('  ' + p);
  console.error('\nRemove the secret (and rotate it if it was real). For a FAKE value in a test fixture,');
  console.error('end that line with the comment: guard-allow-secret\n');
  process.exit(1);
}
console.log(
