...
// exactly the same checks. Plain JavaScript with no dependencies, so it runs before any install step.
import { KEY_PATTERNS } from './key-patterns.mjs';

/** A line ending with this marker holds a fake, key-shaped value on purpose (test fixtures). */
export const ALLOW_MARKER = 'guard-allow-secret';
...
/** `[name, pattern]` pairs, in the order of scripts/key-patterns.mjs. */
...
export const TEXT_EXTENSIONS = new Set(
  '.ts .mts .cts .tsx .js .mjs .cjs .jsx .json .md .mdx .yaml .yml .css .scss .html .astro .vue .svelte .txt .sh .toml .svg'.split(' '),
);
